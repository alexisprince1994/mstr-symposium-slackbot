...
class TestFilteringReport(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.client = MstrClient(project_id=cls.PROJECT_ID, base_url=cls.BASE_URL)
    ...
    def test_create(self, mock_post):
        ...
        response = self.report.create(requests.session(), attribute_id, attribute_name)
    ...
    def test_build_view_filter_no_match(self):
        ...
        with self.assertRaises(ValueError):
            self.report.build_view_filter(
                self.real_example_response, "NOT A REAL ELEMENT VALUE"
            )

        with self.assertRaises(ValueError):
            self.report.build_view_filter(self.real_example_response, 4)
    ...
    def test_build_view_filter_success(self):
        ...
        # a good value.
        self.report.build_view_filter(self.real_example_response, "Books")
...
class TestReport(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.client = MstrClient(project_id=self.PROJECT_ID, base_url=self.BASE_URL)
        # Assuming we're in a logged in state.
...
class ParserTestNoMetrics(unittest.TestCase):
    ...
    def test_response_no_metrics_same_length(self):
        ...
        self.assertEqual(
            len(rows),
            len(expected_response_no_metric),
...
class ParserTestSingleAttribute(ParserBaseClass, unittest.TestCase):
    ...
    def test_response_single_attribute_same_length(self):
        ...
        self.assertEqual(
            len(rows),
            len(expected_response_single_attribute),
            f"Expected rows dont have same row count as parsed rows. \
...
class ParserTestMultipleAttributes(ParserBaseClass, unittest.TestCase):
    ...
    def test_response_multiple_attributes_same_length(self):
        ...
        self.assertEqual(
            len(rows),
            len(expected_response_multiple_attributes),
            f"Expected rows dont have same row count as parsed rows. \
