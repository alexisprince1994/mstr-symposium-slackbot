...
def voteonjoke():
    ...
    print(
        "keys from payload are {}".format(
            ", ".join([key for key in button_event.payload.keys()])
        )
    )
    print(
        "original message attachments are {}".format(
            button_event.original_message["attachments"]
        )
    )
    ...
    response = pybot.respond_to_joke_vote(
        button_event.channel_id, button_event.message_ts, button_event.return_message
    )
