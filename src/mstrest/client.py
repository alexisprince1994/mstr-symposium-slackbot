...
class MstrClient(object):
    ...
    def logout(self):
        ...
        if self.auth_token is None:
            raise TypeError(
                "No auth token found. Cannot log out if not currently logged in."
            )
...
class Report:
    def __init__(
        ...
        *,
        ...
        self.report_definition = {"attributes": {}, "metrics": {}}
    ...
    def create(
        ...
        if self.instance_id is not None:
            raise TypeError(
                "Create called more than once for the same instance.\
        ...
        body = {
            ...
            "requestedObjects": {"attributes": attributes, "metrics": metrics},
        }
        ...
        if body:
            ...
            response = session.post(
                self.report_url, headers=self.request_headers, params=params
            )
    ...
    def fetch(self, session, *, re_read: bool = False):
        ...
        if self.instance_id is None:
            raise TypeError(
                "Instance ID is required to fetch from the instance. \
				Instance ID is auto-generated after using create method."
            )

        if self.page_limit == -1 and not re_read:
            raise ValueError(
                "Page limit was set to read entire report on create. \
...
class FilteringReport(Report):
    ...
    def create(self, session, attribute_id, attribute_name):
        ...
        response = session.post(
            self.report_url, headers=self.request_headers, data=json.dumps(body)
        )
    ...
    def build_view_filter(self, response, element_name):
        ...
        for val, id in self._get_elements(response):
            ...
            raise ValueError(
                "ID not found for attribute element {}".format(element_name)
            )
        ...
        view_filter = FilteringReport._build_view_filter(
            attribute_id, attribute_name, element_id, element_name
        )
    ...
    def _build_view_filter(attribute_id, attribute_name, element_id, element_name):
        ...
        view_filter = {
            "operator": "Equals",
            "operands": [
                {"type": "attribute", "id": attribute_id, "name": attribute_name},
                {
                    "type": "elements",
                    "elements": [{"id": element_id, "name": element_name}],
                },
            ],
        }
