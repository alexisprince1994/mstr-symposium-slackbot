...
class SlackButton(object):
	...
	def __init__(self, name: str, value: str, text: str=None, style: str=None, 
			confirm: dict=None, callback_id: str=None):
		self.name = name
		self.value = value
		self.text = text
		self.style = style
...
class InteractiveRequest(object):
	...
	def _validate_teamid(self, team_id: str):
		...
		if self.team_id != team_id:
			error_msg = 'Unauthorized team usage. Team {} has not been approved'.format(self.team_id)
			return (error_msg, 403)
	...
	def validate_request(self, verification_token: str, team_id: str, callback_id: str, 
...
class InteractiveButtonRequest(InteractiveRequest):
	...
	def _validate_actions(self):
		...
		try:
			...
			raise IndexError('Passed validation check but index value of 0 wasnt found. \
				Self.actions has length {} and I was expecting length 1.'.format(len(self.actions)))
		except KeyError:
			raise KeyError('Slacks API expects there to be a key "value" for the \
...
class DadJokeMessage(object):
	...
	def create_attachment(self):
		...
		good_joke_button = SlackButton(name='goodJokeButton', value='good_joke', text='Great Joke!')
		good_joke_dict = good_joke_button.to_dict()
		bad_joke_button = SlackButton(name='badJokeButton', value='bad_joke', text='Bad Joke!!')
