...
def create_product_analysis(avg_units, stdev_units, *, title=None, text=None):
    ...
    fields = [
        ("Avg Units Last 365", avg_units, "{:,.0f}", "-{:,.0f}"),
...
def create_customer_analysis(overall, regional, *, title=None, text=None):
    ...
    fields = [
        ("Customer Sales Rank (Overall)", overall, "{:,.0f}", "-{:,.0f}"),
...
def create_slack_price_request(app_response, price_ui, product_ui, customer_ui):
    ...
    request_reason = (
        app_response["price_request"]["request_reason"] or "No request reason provided."
    )

    actions = create_action_buttons(app_response["post_route"], app_response["actions"])

    output = {
        "text": "Product Pricing Approval Form",
        "attachments": [
            {
                "title": "{:,.0f}".format(units)
                + " units of {} product requested on {}".format(
...
class SlackPriceAnalysis:
    ...
    def to_json(self):
        self.price_analysis.add_fields(
            self.current_price,
            self.requested_price,
            self.current_margin,
            self.requested_margin,
            self.current_profit,
            self.requested_profit,
        )
...
class SlackActionBar:
    def __init__(self, route, approve_action, deny_action):
        ...
        self.approve_button = SlackButton(
            name=approve_action, value=route, text="Approve", style="primary"
        )
        self.deny_button = SlackButton(
            name=deny_action, value=route, text="Deny", style="danger"
        )
    ...
    def to_json(self):
        return {
            ...
            "actions": [self.approve_button.to_json(), self.deny_button.to_json()],
        }


class SlackButton(object):
    def __init__(
        self, name, value, text=None, style=None, confirm=None, callback_id=None
    ):
        self.name = name
        self.value = value
        self.text = text
