...
def prstatus():
	...
	msg = {'text': 'Thanks!', 'attachments': [{'text': 
		('Thank you for changing the status to {}! '.format(price_request_status) + 
...
def listening():
	...
	if request.form.get('command').lower() in ['/pricerequest', '/pr']:
		...
		if 'help' in request.form.get('text').lower():
			...
			pybot.respond(channel_id, help_message,
				response_type='EPHEMERAL', user_id=user_id)
			return Response(), 200

		placeholder_message = {'text': 'Price Request Analysis', 
		'attachments': 
			[{'attachment_type': 'default', 'text': 'Processing your request!'}]}
		slack_placeholder_message = pybot.respond(channel_id, placeholder_message,
